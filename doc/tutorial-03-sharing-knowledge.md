...
It turns out that the single-role choreographic types (such as `A` and `B`) and the single-arrow communication operator (like `A->B`) are actually degenerate versions of a more powerful idea.
...
The type `#{A B}`, a Clojure set of roles, is called an **agreement type**.
...
The double-arrow communication operator `A=>B` used in the body is like the single-arrow operator, except that instead of just **moving** a value from `A` to `B`, it **copies** it to (or shares it with) `B`.
...
From the above descriptions of agreement and copying, it can be seen that that Klor fundamentally assumes that **copying preserves value**, i.e. that communication will not modify the value in a way that would change the outcome of any deterministic computation using it as input (all else being equal).
...
With the idea of agreement laid out, we can now briefly revisit and expand on the previously introduced concept of role expressions and lifting.
...
`(lifting [A B C] ...)` would lift all literals and free names (referring to plain, i.e. non-choreographic, Clojure vars) within its body to the agreement type `#{A B C}`.
This is the way that Klor reuses existing Clojure code; the "Klor-calls-Clojure" direction of integration.

Without an explicit `lifting` block, Klor behaves as if the whole choreography's body was wrapped in a `lifting` block that includes all of its roles.
...
Notice how the code assumes that the lifted `println` returns a value of agreement type, i.e. that the same value will be returned at both roles.
...
Luckily, Clojure is a language that highly emphasizes stateless and pure functions so ensuring this condition is generally not an issue.
...
The underlying values are still dynamically typed, so if used in the operator position, Klor happily assumes they can be invoked, just like Clojure (and throws a run-time error otherwise).

## Narrowing

Dually to extending agreement, we can also **narrow** it: if `x` is a value of agreement type `#{A B C}`, the special operator `(narrow [A B] x)` will produce the same value but of agreement type `#{A B}`.
This allows us to effectively "forget" a part or the whole of an agreement, which is useful when we want to use the value purely locally or among a subset of the roles.
For example:
...
>> A --> B: 5
>> B exited normally
=> {A #function[klor.multi.runtime/noop], B 6}
```

This copy-then-narrow pattern should immediately remind us of the move-style communication we used early on in the tutorial.
We now see that Klor's fundamental communication mechanism is the double-arrow copy operator `(A=>B <expr>)`, which is syntax sugar for the special operator `(copy [A B] <expr>)`.
...
Specifically, the actual agreement operator rule requires that the operator's arguments are all subtypes of the operator's agreement type.
...
The fundamental problem is known as the **problem of knowledge of choice**: if one role makes a decision regarding which branch of a conditional the choreography should take, how can the other roles involved be notified of its decision?
...
Otherwise, `B` will use the integer `42` instead.

Test with an odd number, `@(simulate-chor check-2 1)`:
...
>> B exited normally
>> A exited normally
=> {A #function[klor.multi.runtime/noop], B 3}
```

Notice the extra communication in the case of an even number.
...
Achieving these slight but sometimes important variations in behavior is quite easy once the groundwork for agreement has been set.

...
Throughout the tutorial we will explore a number of examples, so the reader is encouraged to follow along and try things out at the REPL.
...
See [Reference: Language](./reference-01-language.md) for a full list of special operators supported by Klor, though the most important ones will be covered by the tutorial.

Behind the scenes, the `defchor` macro will type check our choreography and, assuming everything is ok, produce a projection for each of the roles involved.
...
The return value is a map of the results produced by each role (i.e. by each projection).
...
>> B: World
>> B exited normally
=> {A #function[klor.multi.runtime/noop], B nil}
```
...
Also note the projections' results: `B` returned `nil`, which is the result of the choreography's final `println`, but `A` returned the special value `klor.multi.runtime/noop`, which marks the absence of a value for `A` at the choreographic level.
...
>> B exited normally
>> A exited normally
=> {A #function[klor.multi.runtime/noop], B 5}
```
...
Klor won't complain if you use a role expression anyway, and even if the role doesn't match the actual location of the argument:
...
>> A --> B: "Hello"
>> B exited normally
=> {A #function[klor.multi.runtime/noop], B "Hello"}
```
...
(The role expression `(B ...)` serves to lift `inc` to `B`.)
...
>> B --> A: 6
>> A exited normally
=> {A 6, B #function[klor.multi.runtime/noop]}
```
...
To test, `@(simulate-chor remote-invoke dec 5)`:
...
>> B --> A: 4
>> A exited normally
=> {A 4, B #function[klor.multi.runtime/noop]}
```
...
To test, `@(simulate-chor remote-apply + [1 2 3])`:
